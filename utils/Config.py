...
class Config:
    ...
    # Losses weights
    GAN_LOSS_WEIGHT        = 0.5    # Adversarial Loss (GAN): отвечает за реалистичность текстур и детализацию, но слишком большой вес → артефакты. Обычно λ_GAN ∈ [0.1, 10].
    L1_LOSS_WEIGHT         = 100.0   # L1 Loss: стабилизирует обучение, отвечает за грубую цветовую/яркостную точность, смягчает шум, λ_L1 ∈ [10, 200].
    ...
    TV_LOSS_WEIGHT         = 0.3    # Total Variation: сглаживает шум и мелкие артефакты, λ_TV ∈ [1e-8, 0.1].
