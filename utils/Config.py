...
class Config:
    ...
    # Losses weights
    # GAN_LOSS_WEIGHT        = 5    # Adversarial Loss (GAN): отвечает за реалистичность текстур и детализацию, но слишком большой вес → артефакты. Обычно λ_GAN ∈ [0.1, 10].
    # L1_LOSS_WEIGHT         = 80.0   # L1 Loss: стабилизирует обучение, отвечает за грубую цветовую/яркостную точность, смягчает шум, λ_L1 ∈ [10, 200].
    # FM_LOSS_WEIGHT         = 5.0    # Feature Matching: выравнивает фичи разных масштабов, улучшает стабильность, λ_FM ∈ [1, 10].
    # PERCEPTUAL_LOSS_WEIGHT = 0.5    # Perceptual Loss (VGG): сохраняет высокоуровневый контент и стиль, избавляет от размытости, λ_perc ∈ [0.001, 1].
    # LPIPS_LOSS_WEIGHT      = 0.05    # LPIPS: оптимизирует глубокое восприятие сходства, коррелирует с человеческим восприятием, λ_LPIPS ∈ [0.01, 0.1].
    # TV_LOSS_WEIGHT         = 0.5    # Total Variation: сглаживает шум и мелкие артефакты, λ_TV ∈ [1e-8, 0.1].
